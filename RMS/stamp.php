...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $file_name = $_FILES["stamp_file"]["name"];
    $file_tmp = $_FILES["stamp_file"]["tmp_name"];
    $target_path = $upload_dir . basename($file_name);

    if (move_uploaded_file($file_tmp, $target_path)) {
        // File uploaded successfully
        $category_id = 3;
        $category_desc = 'Rubber Stamp';
        ...
        if ($stmt->execute()) {
            echo "<script>alert('Stamp requisition submitted successfully.');</script>";
        } else {
...
<div class="form-container">
    ...
    <form method="post" enctype="multipart/form-data">
       <div class="inline-group">
        ...
        <!-- Upload File -->
        <label>Upload the template for Stamp:</label>
        <input type="file" name="stamp_file" accept=".jpg,.jpeg,.png,.pdf" required>
        <!-- Submit -->
