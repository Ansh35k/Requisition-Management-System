...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $approver = $_SESSION['staff'];
    $update_sql = "UPDATE CPR_REQIUIS_MASTER SET leg_id = ?, acknow_remark = ?, app_by = ? WHERE Req_id = ?";
    $update_stmt = $conn->prepare($update_sql);
    $update_stmt->bind_param("issi", $new_leg_id, $remark, $approver, $req_id);
...
<html>
<head>
    <title>Approval - Visiting Card</title>
    <style>
        body {
            display: flex;
...
<div class="form-container">
    <h2>APPROVAL -  VISITING CARD</h2>
    
    <label>User Name & Staff No.:</label>
    <div class="field"><?= htmlspecialchars($data['user_name_staff_no']) ?></div>

    <label>Category:</label>
    <div class="field"><?= htmlspecialchars(ucfirst($data['sub_category'])) ?></div>

    <label>Contact Details:</label>
    <div class="field"><?= htmlspecialchars($data['contact_details']) ?></div>

    <label>Justification:</label>
    <div class="field"><?= nl2br(htmlspecialchars($data['purpose_justif'])) ?></div>

    <label>Uploaded File:</label>
    <div class="field">
        <a href="uploads/<?= htmlspecialchars($data['item_desc']) ?>" target="_blank">
