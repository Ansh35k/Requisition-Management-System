...
foreach ($categories as $id => $name) {
    $stmt = $conn->prepare("SELECT COUNT(*) FROM CPR_REQIUIS_MASTER WHERE category_id = ? AND leg_id = 1");
...
<h2>Pending for Approval (CPR Only)</h2>

<div class="link-container">
    <?php foreach ($categories as $id => $name): ?>
        <a class="category-link" href="approval_list_<?= strtolower(str_replace(' ', '', $name)) ?>.php">
            <span><?= $name ?></span>
